...
Verify PostgreSQL reports ready and Teem has started without configuration or hash errors. `curl -i https://teem.example.com/` should redirect to `/login`, and `curl -i -u 'operator:YOUR_PASSWORD' https://teem.example.com/` should return the Runs page. There is no `/health` endpoint and the server does not implement HEAD, so use GET. Then run the [slice-4 manual acceptance](../../docs/architecture/vertical-slice-4.md#acceptance): message the bot, have it propose work on a scratch repository, approve it, and confirm that a pull request opens and the check-in arrives on your phone.
